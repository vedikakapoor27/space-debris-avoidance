// ── API Utility ────────────────────────────────────────────────────────────────
// Connects to your Flask backend running on http://localhost:5000
...
/**
 * Send orbital data and get collision risk + avoidance plan
 * POST /predict
 * @param {number} distance_km
 * @param {number} rel_velocity
 * @param {number} approach_rate
 */
...
/**
 * Simulate a stream of debris objects for the 3D globe
 * (mock data — replace with real TLE endpoint when ready)
 */
export function generateDebrisField(count = 80) {
  return Array.from({ length: count }, (_, i) => {
    const theta = Math.random() * Math.PI * 2
    const phi   = Math.acos(2 * Math.random() - 1)
    const r     = 1.3 + Math.random() * 1.2   // orbital radii
    return {
      id:       i,
...
/**
 * Recent conjunction events mock (replace with /conjunctions endpoint later)
 */
